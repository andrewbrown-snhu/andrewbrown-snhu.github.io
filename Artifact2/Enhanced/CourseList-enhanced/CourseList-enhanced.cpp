...
// Changes     : Updated sorting algorithm to use merge sort instead of quicksort which
//                  improves the time complexity from O(n2) to O(n*log(n)). Also added a
//                  variable to track whether the list has already been sorted to optimize
//                  performance.
//============================================================================
...
// Setup the Course List class to store a collection of courses
class CourseList {
private:
    vector<Course> courseList;
    // V2: Added variable for tracking the sort and sorting methods
    bool coursesSorted;
    void sortCourses();
...
CourseList::CourseList() {
    // V2: Set the courseSorted to false when the object is created
    coursesSorted = false;
}
...
/**
 * V2: Private method - Split the courseList vector in half and then
        sort the items based on course number

 * @param start - the starting position
 * @param mid - the middle position
 * @param end - the ending position
 */
void CourseList::merge(int start, int mid, int end) {
    ...
    // Create temp vectors
    vector<Course> leftPart(n1), rightPart(n2);

    // Move values for temporary storage
    for (i = 0; i < n1; i++)
        leftPart[i] = courseList[start + i];
    for (j = 0; j < n2; j++)
        rightPart[j] = courseList[mid + 1 + j];

    // Update main courseList vector with temporary storage values
    i = 0;
    j = 0;
    k = start;
    while (i < n1 && j < n2) {
        if (leftPart[i].number <= rightPart[j].number) {
            courseList[k] = leftPart[i];
            i++;
        }
        else {
            courseList[k] = rightPart[j];
            j++;
        }
        k++;
    }
...
/**
 * V2: Private method - Recursive Merge Sort algorithm that works
 *      through all vector items until they are fully sorted
 * 
 * @param start - the starting position
 * @param end - the ending position
 */
void CourseList::mergeSort(int start, int end) {
    if (start < end) {
...
/**
 * V2: Private method - Starts sorting the courses by calling the main sort
 *      method. Using mergeSort for this size of data improves the time complexity from O(n2) to O(n*log(n)).
 */
void CourseList::sortCourses() {
    mergeSort(0, courseList.size() - 1);
}
...
void CourseList::addCourse(string number, string name) {
    ...
    // V2: Force re-sorting the courses since a new item is added
    coursesSorted = false;
}
...
void CourseList::printCourses() {
    // V2: Added checking to see if the courses were already sorted
    if (!coursesSorted) {

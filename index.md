...
The work completed in previous courses demonstrates how the skills learned in the courses are applied to build a running application. When collaborating in a team environment, it is important to add comments in the code explaining what actions the section of code will complete in the application. If the section is a function that will be called somewhere else in the code, the comment should provide the inputs required for the function and the output that it will return. Comments were added to provide all of this information for any code written in the previous courses.

When communicating with stakeholders, different levels of information should be provided. Stakeholders are generally interested in the value the application will provide to the business instead of the technical details within the application. When presenting the application to stakeholders, the information they will want to know is the problem that currently exists in the business and how the application will solve that problem. They will also need to know the limitations and constraints of the application, such as the platform required, the information it will need to access, and budget costs.

Data Structures are important for any language that exists today, including ones that are loosely typed, one that does not require a type to be defined for variables.  The computer and sequently the programmer need to know what kind of data will be stored and this data may also be transferred to another program which will have certain data type requirements. Algorithms are also important to efficiently calculate the data in a manner that is readable to a human and doesn’t require code to be duplicated. Most programs have some type of algorithms that the computer will compute some results based on a repetitive task and having an understanding of the inner working for algorithms that exist today will help implement what already exists instead of rewriting the same code again.

In Software Engineering and Database, all software goes through an iterative process where business requirements are gathered. Components and data stores can be derived from those requirements, and each one will be responsible for handling a specific task within the requirements. Once an overall structure is established, pseudocode can be written and validated against the requirements before it is turned into code. 

Security is always a top priority for any code that is written. Some programs will handle sensitive data, and security controls will need to be built in to prevent unauthorized access to that data. Access Control List and Authentication are two ways to implement security to restrict access, but the code should also be tested against common security vulnerabilities to reduce the chance that a malicious user will bypass the security measures.

The artifacts presented in the portfolio present applied computer science skills in running applications. For each artifact, an original version is presented along with the enhanced version, which will show changes in the code to demonstrate a specific outcome. In the first artifact, the Text Based Game was rewritten in JavaScript to increase portability. The second artifact optimizes the sorting algorithm for the Course List. The third artifact adds additional features for users to update their profiles and add new users. All three of the artifact enhancements show skills within multiple areas of computer science.
